...
<body>
  ...
  <main class="layout">
    ...
    <section id="panel-admin" class="card" style="display:none; grid-column:1/-1;">
      ...
      <div style="display:flex; flex-direction:column; gap:8px; max-width:400px;">
        ...
        <button id="btnChangePass">Cambiar contraseña</button>
      </div>
    </section>
  </main>
  ...
  <script>
    ...
    btnChangePass.onclick=()=>{ 
      const s=getSession(); if(!s) return alert("No hay sesión activa");
      const u=s.user; 
      if(admins[u].password!==oldPass.value) return alert("Contraseña actual incorrecta");
      if(!newPass.value) return alert("Escribe una nueva contraseña");
      admins[u].password=newPass.value; save(KEY_ADMINS,admins);
    ...
    // --- Exportación con columnas completas ---
    btnExport.onclick=()=>{ 
      let map={}; scans.forEach(s=>map[s.code]=(map[s.code]||0)+1);
      let csv='Código,Nombre,Fecha,Hora,Veces\n';
      scans.forEach(s=>{ 
        const d=s.ts.split('T'); 
        csv+=`${s.code},"${people[s.code]||''}",${d[0]},${d[1].slice(0,8)},${map[s.code]}\n`; 
      }); 
